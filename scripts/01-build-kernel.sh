...
# Page-size variant (default 4k). 64k flips the ARM64 page-size choice + tags LOCALVERSION so the kernel
# installs as $KVER-64k beside the 4k build (no collision). olddefconfig recomputes PAGE_SHIFT/PGTABLE_LEVELS.
SFX=""; if [ "$PAGE_SIZE" = 64k ]; then
  scripts/config --disable ARM64_4K_PAGES --enable ARM64_64K_PAGES --set-str LOCALVERSION "-64k"
  SFX="-64k"; echo "[build] 64k page-size variant -> LOCALVERSION=-64k"
fi
make olddefconfig >/work/olddefconfig-$KVER$SFX.log 2>&1
...
echo "===BUILD-START==="
make -j"$(nproc)" Image modules >/work/build-$KVER$SFX.log 2>&1
echo "BUILD-RC=$?"
[ -f arch/arm64/boot/Image ] && echo "KERNEL-BUILT $KVER$SFX ($(ls -la arch/arm64/boot/Image | awk "{print \$5}") bytes)"
cp .config /work/config-$KVER$SFX
'
echo "KERNEL-JOB-DONE $KREL ($PAGE_SIZE pages)"
