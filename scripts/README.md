...
| Script | Produces |
|---|---|
| `01-build-kernel.sh` | stock upstream `$KVER` (from `versions.env`; currently 6.18.37) + `config/rocky-6.18.34-gb10.config` (base config — carries forward to newer `$KVER` via `olddefconfig`; the resolved `config-$KVER` is what the manifest hashes), built in `rockylinux:10` |
