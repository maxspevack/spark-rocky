...
| `reproduce-Nemotron-3-Super-NVFP4-probes-2026-07-25.txt` | **The flagship probe receipt** (#73): the published 23.71 "no-MTP" board-best actually runs MTP nst=1 (premise corrected from its own recipe); true no-MTP baseline 16.48 ± 0.002 reproduced; MTP worth +41–56%; acceptance curve 1.61/2.18/2.64 monotonic in nst. |
| `reproduce-Nemotron-3-Super-NVFP4-mtp-2026-07-26.txt` + `nemotron-3-super-nvfp4-mtp-matrix-2026-07-26.csv` | **The flagship receipt** (#73): headline `tg128 (c1)` N=5 = **23.57 ± 1.74 CLEAN** — statistically tied with board-best single-node; 14/28 cells receipt-grade; the other 14 are the **measured per-cell cooling boundary** (all but the five heaviest re-attempted as single cells and still throttled) with every trace and verdict recorded, incl. the #62 hang. |
