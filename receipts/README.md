...
| `reproduce-Qwen3.5-0.8B-clk4k-2026-07-23.txt` + `qwen3.5-0.8b-matrix-clk4k-2026-07-23.csv` | The **CLK default is benchmark-validated** (#61): full matrix on `6.18.38-clk`/4k, **median 1.007× vs the June stock-host baseline** — the shipped kernel reproduces the published numbers, not just the stock host. (Caveat inside: directional-strong, not throttle-certified.) |
| `reproduce-Qwen3.5-0.8B-gen2-2026-07-24.txt` + `qwen3.5-0.8b-matrix-gen2-2026-07-24.csv` | **Parity holds on the current runtime** (#71): full matrix on `6.18.39-clk` + the pinned dgx-vllm mirror image (vLLM 0.23.1), **median 1.010× vs published** — decode 1.010×, prefill 1.011× (the June prefill deficit was runtime drift; #18 holds the localized residual). Throttle-check CLEAN. |
