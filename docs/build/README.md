...
- [`build.md`](build.md) — the `01`→`04` pipeline and prerequisites, **the kernel-as-RPM mechanism (#59/#77), the RPM accounting** (every byte on the box, named), **the release runbook** (package, sign, tag, verify), and **the debug hatch**. **Start here.**
...
- [`downstream.md`](downstream.md) — building **on** spark-rocky as a downstream: the signed-release consumption contract (pin + verify + extend) + the `01`→`07` divergence seams (kernel / packages / validation).

Adopted upstreams, version pins, and the zero-patch accounting live at the repo root:
[`THIRD_PARTY.md`](../../THIRD_PARTY.md) (the supply-chain map — one canonical file, test-enforced).
