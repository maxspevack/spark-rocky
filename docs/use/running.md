...
A signed Live USB image runs Rocky Linux 10.2, the **CIQ Linux Kernel — CLK 6.18 with 4k pages** (`uname -r` → `6.18.39-clk`, the suffix states the kernel lineage; `getconf PAGESIZE` → `4096`), and the open NVIDIA driver 610.43.03 on the GB10. **Non-destructive: it boots from the USB only; your internal NVMe (DGX OS, models, data) is not mounted or written.** Three steps: flash, boot, check.
